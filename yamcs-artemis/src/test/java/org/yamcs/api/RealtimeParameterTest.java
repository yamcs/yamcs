...
public class RealtimeParameterTest {
    ...
    void testSubscribeSome() throws Exception {
        ...
        yclient.dataConsumer.setMessageHandler(msg -> decodeAndPrint(msg));
    ...
    static void decodeAndPrint(ClientMessage msg) {
        try {
            ...
            for (int i = 0; i < pdata.getParameterCount(); i++) {
                ...
                System.out.println(String.format("%-25s %-30s %s", TimeEncoding.toString(pv.getAcquisitionTime()),
                        pv.getId().getName(), StringConverter.toString(pv.getEngValue())));
