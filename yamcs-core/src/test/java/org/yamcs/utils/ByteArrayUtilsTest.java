...
public class ByteArrayUtilsTest {
    ...
    @Test
    public void testEnforcedUnsignedIntDecode() {
        byte[] b = new byte[8];
        long l = 0xFE010203L;
        ByteBuffer.wrap(b).putLong(l);
        long l1 = ByteArrayUtils.decodeUnsignedInt(b, 4);
        assertEquals(l, l1);
    }

    @Test
    public void testEnforcedUnsignedIntLEDecode() {
        byte[] b = new byte[8];
        long l = 0xFE010203L;
        ByteBuffer.wrap(b).order(ByteOrder.LITTLE_ENDIAN).putLong(l);
        long l1 = ByteArrayUtils.decodeUnsignedIntLE(b, 0);
        assertEquals(l, l1);
    }
}
