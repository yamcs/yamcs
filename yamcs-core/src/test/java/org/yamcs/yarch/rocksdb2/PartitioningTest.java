...
public class PartitioningTest extends YarchTestCase {
    ...
    public void testIndexPartitioning() throws ParseException, StreamSqlException, IOException, InterruptedException {
        ...
        ydb.execute("create table test1(gentime timestamp, apidSeqCount int, primary key(gentime,apidSeqCount)) engine rocksdb2 partition by time(gentime('YYYY/DOY'))");
    ...
    public void testDoublePartitioning() throws Exception {
        ...
        ydb.execute("create table testdp(gentime timestamp, seqNumber int, part enum, packet binary, primary key(gentime,seqNumber)) engine rocksdb2 partition by time_and_value(gentime('YYYY/DOY'), part) ");
        ...
        Iterator<Partition>it = partitions.iterator();
        assertEquals(2, partitions.size());
        RdbPartition p0 = (RdbPartition)it.next();
        assertEquals("1999/172/testdp", p0.dir);
       
        
        RdbPartition p1 = (RdbPartition)it.next();
        assertEquals("1999/172/testdp",p1.dir);
        
        Object[] pvalues = new Object[]{p0.getValue(), p1.getValue()};
        Arrays.sort(pvalues);
        ...
        assertEquals("2001/001/testdp",p2.dir);
