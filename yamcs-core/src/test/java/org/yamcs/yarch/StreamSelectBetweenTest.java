...
public class StreamSelectBetweenTest extends YarchTestCase {
    int blength = 100;
    int n = 1000;
    ...
    public void testFilterBetween() throws Exception {
        ...
        new StreamChecker("tm_out1",
                "select * from tm_in where gentime between '2025-09-02T15:30:00' and '2025-09-02T15:30:05'",
                checker);
        Stream s = ydb.getStream("tm_in");
        feed(s, t);
        s.close();
        // we have one more than the AND test because BETWEEN is inclusive on both ends
        assertEquals(66, checker.x);
    }
    ...
    public void testFilterNotBetween() throws Exception {
        ...
        var checker = new TupleChecker() {
            ...
            public void check(int count, Instant time, int id) {
                ...
                // Should match everything EXCEPT the 6 seconds from 15:30:00 to 15:30:05
                // So exclude times: t+60000, t+61000, t+62000, t+63000, t+64000, t+65000
                if (millis >= t + 60000 && millis <= t + 65000) {
                    throw new AssertionError("Time " + time + " should be excluded by NOT BETWEEN");
                }
                expectedCount++;
            }
        };
        ...
        new StreamChecker("tm_out1",
                "select * from tm_in where gentime not between '2025-09-02T15:30:00' and '2025-09-02T15:30:05'",
                checker);
        Stream s = ydb.getStream("tm_in");
        feed(s, t);
        s.close();
        // Total 1000 records - 6 records in the BETWEEN range = 994
        assertEquals(994, checker.expectedCount);
    }
