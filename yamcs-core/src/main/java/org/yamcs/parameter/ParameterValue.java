...
public class ParameterValue extends BasicParameterValue {
    ...
    public int getSerializedSize(int numericId) {
        int size = memoizedSize;
        if (size != -1) {
            return size + CodedOutputStream.computeUInt32Size(NUMERICID_FN, numericId);
        }
        ...
        if (status.getAcquisitionStatus() != null) {
            size += CodedOutputStream.computeEnumSize(ACQUISITIONSTATUS_FN, status.getAcquisitionStatus().getNumber());
        }

        if (status.getMonitoringResult() != null) {
            size += CodedOutputStream.computeEnumSize(MONITORINGRESULT_FN, status.getMonitoringResult().getNumber());
        }
    ...
    public void writeTo(CodedOutputStream output, int numericId) throws IOException {
        ...
        output.writeUInt32(27, numericId);
