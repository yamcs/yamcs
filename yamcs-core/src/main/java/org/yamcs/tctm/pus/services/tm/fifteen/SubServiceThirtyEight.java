...
public class SubServiceThirtyEight implements PusSubService {
    String yamcsInstance;
    YConfiguration config;


    public SubServiceThirtyEight(String yamcsInstance, YConfiguration config) {
    ...
    public ArrayList<TmPacket> process(TmPacket tmPacket) {
        ...
        int applicationServiceId, diagnosticStructureId;
        ArrayList<byte[]> bytestream = new ArrayList<>();

        for(int i = 0; i < n1 ; i++) {
            int lengthI = ServiceFifteen.applicationIdSize + ServiceFifteen.n2Size + n2 * ServiceFifteen.diagnosticStructureIdSize;
            ...
            for (int j = 0; j < n2; j++) {
                int indexJ = j * ServiceFifteen.diagnosticStructureIdSize;
                diagnosticStructureId = (int) ByteArrayUtils.decodeCustomInteger(dataField, ServiceFifteen.packetStoreIdSize + ServiceFifteen.n1Size + ServiceFifteen.applicationIdSize + ServiceFifteen.n2Size + indexI + indexJ, ServiceFifteen.diagnosticStructureIdSize);

                // Add to bytestream | To be made into a new TmPacket
                ByteBuffer bb = ByteBuffer.wrap(new byte[ServiceFifteen.packetStoreIdSize + ServiceFifteen.applicationIdSize + ServiceFifteen.diagnosticStructureIdSize]);
                ...
                bb.put(ByteArrayUtils.encodeCustomInteger((long) diagnosticStructureId, ServiceFifteen.diagnosticStructureIdSize));
        ...
        ByteBuffer bb = ByteBuffer.wrap(new byte[primaryHeader.length + secondaryHeader.length + ServiceFifteen.numerationsCountSize + (ServiceFifteen.packetStoreIdSize + ServiceFifteen.applicationIdSize + ServiceFifteen.diagnosticStructureIdSize) * bytestream.size()]);

        // Add a 2 byte field which carries the number of (packetStoreId, Apid, diagnosticStructureId) numerations, which will be used in the MDb to extract the values
