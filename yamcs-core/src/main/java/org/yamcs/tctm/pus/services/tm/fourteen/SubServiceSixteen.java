...
public class SubServiceSixteen implements PusSubService {
    ...
    public SubServiceSixteen(String yamcsInstance, YConfiguration config) {
    ...
    public ArrayList<TmPacket> process(TmPacket tmPacket) {
        ...
        for(int i = 0; i < n1 ; i++) {
            int lengthI = ServiceFourteen.applicationIdSize + ServiceFourteen.n2Size + n2 * ServiceFourteen.eventDefinitionIdSize;
            ...
            for (int j = 0; j < n2; j++) {
                int indexJ = j * ServiceFourteen.eventDefinitionIdSize;
                eventDefinitionId = (int) ByteArrayUtils.decodeCustomInteger(dataField, ServiceFourteen.n1Size + ServiceFourteen.applicationIdSize + ServiceFourteen.n2Size + indexI + indexJ, ServiceFourteen.eventDefinitionIdSize);

                // Add to bytestream | To be made into a new TmPacket
                ByteBuffer bb = ByteBuffer.wrap(new byte[ServiceFourteen.applicationIdSize + ServiceFourteen.eventDefinitionIdSize]);

                bb.put(ByteArrayUtils.encodeCustomInteger((long) applicationServiceId, ServiceFourteen.applicationIdSize));
                bb.put(ByteArrayUtils.encodeCustomInteger((long) eventDefinitionId, ServiceFourteen.eventDefinitionIdSize));
        ...
        ByteBuffer bb = ByteBuffer.wrap(new byte[primaryHeader.length + secondaryHeader.length + ServiceFourteen.numerationsCountSize + (ServiceFourteen.applicationIdSize + ServiceFourteen.eventDefinitionIdSize) * bytestream.size()]);

        // Add a 2 byte field which carries the number of (Apid, eventDefinitionId) numerations, which will be used in the MDb to extract the values
