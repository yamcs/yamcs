...
public class SubServiceTwelve implements PusSubService {
    ...
    public SubServiceTwelve(String yamcsInstance, YConfiguration config) {
    ...
    public ArrayList<TmPacket> process(TmPacket tmPacket) {
        ...
        for(int i = 0; i < n1 ; i++) {
            int lengthI = ServiceFourteen.applicationIdSize + ServiceFourteen.n2Size + n2 * ServiceFourteen.diagnosticStructureIdSize;
            ...
            for (int j = 0; j < n2; j++) {
                int indexJ = j * ServiceFourteen.diagnosticStructureIdSize;
                diagnosticStructureId = (int) ByteArrayUtils.decodeCustomInteger(dataField, ServiceFourteen.n1Size + ServiceFourteen.applicationIdSize + ServiceFourteen.n2Size + indexI + indexJ, ServiceFourteen.diagnosticStructureIdSize);

                // Add to bytestream | To be made into a new TmPacket
                ByteBuffer bb = ByteBuffer.wrap(new byte[ServiceFourteen.applicationIdSize + ServiceFourteen.diagnosticStructureIdSize]);

                bb.put(ByteArrayUtils.encodeCustomInteger((long) applicationServiceId, ServiceFourteen.applicationIdSize));
                bb.put(ByteArrayUtils.encodeCustomInteger((long) diagnosticStructureId, ServiceFourteen.diagnosticStructureIdSize));
        ...
        ByteBuffer bb = ByteBuffer.wrap(new byte[primaryHeader.length + secondaryHeader.length + ServiceFourteen.numerationsCountSize + (ServiceFourteen.applicationIdSize + ServiceFourteen.diagnosticStructureIdSize) * bytestream.size()]);

        // Add a 2 byte field which carries the number of (Apid, diagnosticStructureId) numerations, which will be used in the MDb to extract the values
