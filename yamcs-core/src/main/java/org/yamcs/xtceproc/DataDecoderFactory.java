...
public class DataDecoderFactory {
    ...
    public static DataDecoder get(Algorithm a) {
        if(!(a instanceof CustomAlgorithm)) {
            throw new XtceProcException("Unsupported algorithm: '"+a+"'. Only Java custom algorithms supported");
        }
        CustomAlgorithm ca = (CustomAlgorithm)a;

        if(!"java".equals(ca.getLanguage().toLowerCase())) {
            throw new XtceProcException("Unsupported language for Data Decoder: '"+ca.getLanguage()+"'. Only Java supported");
        }
    ...
    private static DataDecoder loadJavaDecoder(CustomAlgorithm alg, AlgorithmExecutionContext execCtx) {
        ...
        if(!m.matches()) {
            throw new XtceProcException("Cannot parse algorithm text '"+alg.getAlgorithmText()+"'");
        }
