...
public class ParameterTypeProcessor {
    ...
    private void doCalibrate(ProcessingData processingData, ParameterValue pval) {
        ...
        try {
            if (requireCalibration(ptype, rawValue)) {
                engValue = doCalibrate(processingData, ptype, rawValue);
            } else {
    ...
    private Value doCalibrate(ProcessingData processingData, ParameterType ptype,
        ...
        if (ptype instanceof BaseDataType bdt) {
            CalibratorProc calibrator = pdata.getCalibrator(processingData, bdt);
            if (calibrator == null) {
                if (ptype instanceof EnumeratedParameterType ept) {
                    ...
                    engValue = convertToInteger(processingData, ipt, rawValue);
                } else if (ptype instanceof FloatParameterType fpt) {
                    engValue = convertToFloat(processingData, fpt, rawValue);
                    ...
                    engValue = calibrateAbsoluteTime(processingData, apt, rawValue);
                } else {
                    throw new IllegalStateException("Extraction of " + ptype + " not implemented");
                }
            } else {
                engValue = calibrator.calibrate(rawValue, processingData);
            }
        } else if (ptype instanceof AggregateParameterType apt) {
            engValue = calibrateAggregate(processingData, apt, (AggregateValue) rawValue);
        } else if (ptype instanceof ArrayParameterType apt) {
            engValue = calibrateArray(processingData, apt, (ArrayValue) rawValue);
        } else {
            throw new IllegalStateException("Extraction of " + ptype + " not implemented");
        }
        return engValue;
    ...
    private Value convertToInteger(ProcessingData processingData, IntegerParameterType ipt, Value rawValue) {
        switch (rawValue.getType()) {
        case SINT32:
            return convertToInterger(processingData, ipt, rawValue.getSint32Value());
        case SINT64:
            return convertToInterger(processingData, ipt, rawValue.getSint64Value());
        case UINT32:
            return convertToInterger(processingData, ipt, rawValue.getUint32Value() & 0xFFFFFFFFL);
        case UINT64:
            return convertToInterger(processingData, ipt, rawValue.getUint64Value());
        case FLOAT:
            return convertToInterger(processingData, ipt, (long) rawValue.getFloatValue());
        case DOUBLE:
            return convertToInterger(processingData, ipt, (long) rawValue.getDoubleValue());
        case STRING:
            try {
                long l = Long.decode(rawValue.getStringValue());
                return convertToInterger(processingData, ipt, l);
            } catch (NumberFormatException e) {
                try {
    ...
    private Value convertToInterger(ProcessingData processingData, IntegerParameterType ipt, long longValue) {
    ...
    private Value convertToFloat(ProcessingData processingData, FloatParameterType ptype, Value rawValue) {
        switch (rawValue.getType()) {
        case DOUBLE:
            return convertToFloat(processingData, ptype, rawValue.getDoubleValue());
        case FLOAT:
            return convertToFloat(processingData, ptype, rawValue.getFloatValue());
        case SINT32:
            return convertToFloat(processingData, ptype, rawValue.getSint32Value());
        case SINT64:
            return convertToFloat(processingData, ptype, rawValue.getSint64Value());
        case UINT32:
            return convertToFloat(processingData, ptype, rawValue.getUint32Value() & 0xFFFFFFFFL);
        case UINT64:
            return convertToFloat(processingData, ptype, UnsignedLong.toDouble(rawValue.getUint64Value()));
        case STRING:
            try {
                Double d = Double.parseDouble(rawValue.getStringValue());
                return convertToFloat(processingData, ptype, d);
            } catch (NumberFormatException e) {
    ...
    private Value convertToFloat(ProcessingData processingData, FloatParameterType ptype,
            double doubleValue) {
