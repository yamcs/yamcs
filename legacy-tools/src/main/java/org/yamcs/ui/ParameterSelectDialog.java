...
/**
 * Displays packets with their parameters in a hierarchical view, allowing the user to select parameters.
 * 
 * Basic search facility restricts view to parameters which contain specified text, with mutliple terms separated by *.
 * 
 * Tree behaviour is currently default, so selecting a packet will not automatically select all its parameters.
 * 
 * @author atu
 *
 */
public class ParameterSelectDialog extends JDialog implements ActionListener, KeyListener, TreeSelectionListener {
    ...
    /**
     * Gets all selected ParameterEntry instances, ignoring any other selected object.
     * 
