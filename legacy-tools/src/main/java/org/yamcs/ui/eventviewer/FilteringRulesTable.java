...
/**
 * Table with filtering rules. The rules are matched from top to bottom, there is always default rule. If none of the
 * rules does apply for the event, then the default rule is used. All rules are stored in the Preferences backing store.
 */
...
public class FilteringRulesTable extends AbstractTableModel {
    ...
    /** Table columns */
    static final String[] columnNames = { "Active", "Rule name", "Source", "Event Type", "Event Message", "Severity",
            "Alert", "Show" };
    ...
    /**
     * Observable - I have to use aggregation because no multiply inheritance is possible
     */
    ...
    /**
     * Add rule into the list of rules. New rule is added on the beginning of the list.
     * 
     * @param rule
     *            Rule to be added
     */
     ...
     * 
     * @param index
     *            Index of the rule to be removed
     */
     ...
     * 
     * @return Vector containing names of all filtering rules in the same order as the rules are applied.
     */
    ...
    /**
     * Match event against the filtering table and find the rule which matches that event. No other action is done, only
     * the rule is determined. The application of the rule is on the caller.
     * 
     * @param event
     *            Event matched against the filtering table
