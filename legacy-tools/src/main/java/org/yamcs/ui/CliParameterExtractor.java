...
public class CliParameterExtractor {
    ...
    static void printUsageAndExit(String error) {
        ...
        System.err.println(
                "Usage: parameter-extractor.sh [OPTIONS] yamcs-url startTime stopTime [parameter_opsnames | -f file_with_parameters]");
    ...
    /**
     * @param args
     *            http://aces-test:8090/aces-test 2010-05-19T00:00:00 2011-05-20T00:00:00 aces_SHM_HP_SET
     *            aces_SHM_HP_MEAS aces_SHM_IS_HEATER
    ...
    public static void main(String[] args)
            throws ExecutionException, URISyntaxException, FileNotFoundException, IOException, InterruptedException {
        if (args.length < 1) {
        ...
        while (args[k].startsWith("-")) {
            if (args[k].equals("-t")) {
                ...
                printUsageAndExit("Unknown option '" + args[k] + "'");
            }
            k++;
        }
        ...
        if (ycd.getInstance() == null) {
            printUsageAndExit(
                    "The Yamcs URL does not contain the archive instance. Use something like http://hostname/archiveInstance");
        }
        ...
        final ParameterFormatter pf = new ParameterFormatter(new BufferedWriter(new PrintWriter(System.out)),
                prr.getIdList());
        ...
        CompletableFuture<Void> completableFuture = restClient.doBulkGetRequest(
                "/archive/" + ycd.getInstance() + "/downloads/parameters", prr.build().toByteArray(), data -> {
                    ParameterData pd;
                    try {
                        pd = ParameterData.parseFrom(data);
                        pf.writeParameters(pd.getParameterList());
                    } catch (InvalidProtocolBufferException e1) {
                        System.err.println("Cannot decode parameter message: " + e1);
                    } catch (IOException e2) {
                        System.err.println("Error when saving parameters: " + e2);
                    }
                });
