...
/**
 * @author nm GUI for requesting parameter dumps
 */
...
public class ParameterRetrievalGui extends JFrame implements ParameterSelectDialogListener {
    ...
    public ParameterRetrievalGui(YamcsConnectionProperties connectionParameters, Component parent) {
        ...
        optionsPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Options"),
        ...
        paramPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Parameters"),
                BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        ...
        TextAreaListener listener = new TextAreaListener() {
            ...
            public void updated() {
                startButton.setEnabled(
                        (!paramText.getText().equalsIgnoreCase(""))
        ...
        button.addActionListener(arg0 -> loadParameters());
        parambuttonPanel.add(button);

        button = new JButton("Save");
        button.addActionListener(arg0 -> saveParameters());
        parambuttonPanel.add(button);

        recentPopup = new JPopupMenu();
        recentButton = new JButton("Recent");
        populateRecentMenu();
        recentButton.addActionListener(event -> recentPopup.show(recentButton, 0, recentButton.getSize().height));
        ...
        selectParamButton.addActionListener(e -> selectParameters());
        ...
        outputPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Output File"),
                BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        fileNameTextField = new JTextField();
        outputPanel.add(fileNameTextField);
        button = new JButton("Choose");
        button.addActionListener(arg0 -> chooseOutputFile());
        ...
        startButton.addActionListener(arg0 -> startRetrieval());
        button = new JButton("Close");
        actionbuttonPanel.add(button);
        button.addActionListener(arg0 -> setVisible(false));
    ...
    /**
     * Convenience method to add opsnames to the displayed list without duplicating any existing entries.
     * 
     * @param opsnames
     */
    ...
    public void setValues(String archiveInstance, long start, long stop) {
        ...
        fileNameTextField
                .setText(String.format("parameters_%s_%s.dump", startWinCompatibleDateTime, stopWinCompatibleDateTime));
    }
    ...
    public static List<NamedObjectId> loadParameters(BufferedReader reader) throws IOException {
        List<NamedObjectId> paramList = new ArrayList<>();
        String line = reader.readLine();

        if (line == null) {
            return paramList; // hmm, empty file
        }
        if (line.contains("<")) {
            // read an xml file
    ...
    private void chooseOutputFile() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (outputFile.exists()) {
                if (JOptionPane.showConfirmDialog(this, "Are you sure you want to overwrite " + outputFile, getTitle(),
    ...
    private void startRetrieval() {
        ...
        setVisible(false);
        progressMonitor = new ProgressMonitor(parent, "Saving parameters", "0 lines saved", 0,
                (int) ((stop - start) / 1000));
        try {
            ...
            if (timeWindow.isSelected()) {
                try {
                    ...
                    showErrorMessage(
                            "Cannot parse number: " + e.getMessage() + ". Please make sure that the number is integer");
                }
            } else {
            ...
            BulkDownloadParameterValueRequest prr = BulkDownloadParameterValueRequest.newBuilder().addAllId(paramList)
                    .setStart(TimeEncoding.toString(start)).setStop(TimeEncoding.toString(stop)).build();
            completableFuture = restClient.doBulkGetRequest("/archive/" + archiveInstance + "/downloads/parameters",
                    prr.toByteArray(), data -> {
                        ParameterData pd;
        ...
        SwingUtilities.invokeLater(
                () -> {
                    if (progressMonitor.isCanceled()) {
                        ...
                        showMessage("The parameter retrieval finished successfully. " + getNote() + " in "
