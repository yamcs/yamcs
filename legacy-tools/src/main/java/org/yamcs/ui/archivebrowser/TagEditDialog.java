...
public class TagEditDialog extends JDialog implements ActionListener {
        ...
        if (e.getActionCommand().equalsIgnoreCase("ok")) {
            ...
            try {
                ...
                if (start == TimeEncoding.INVALID_INSTANT && stop == TimeEncoding.INVALID_INSTANT) {
                    JOptionPane.showMessageDialog(this, "At least one of start or stop has to be specified",
                            "Please specify the start or stop", JOptionPane.ERROR_MESSAGE);
                    return;

                }
                if (start != TimeEncoding.INVALID_INSTANT && stop != TimeEncoding.INVALID_INSTANT &&
                        start > stop) {
                    JOptionPane.showMessageDialog(this, "Stop has to be greater than start", "Invalid times",
                ...
                e1.printStackTrace();
                JOptionPane.showMessageDialog(this, "Error parsing time: " + e1.getMessage(), "Error parsing time",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ok = true;
    ...
    private JComboBox<String> getColorComboBox() {
        if (colorComboBox == null) {
            colorComboBox = new JComboBox<>(ColorUtils.colors.keySet().toArray(new String[0]));
        ...
        if (input instanceof JFormattedTextField) {
            ...
            if (formatter != null) {
                ...
                try {
                    ...
                    pe.printStackTrace();
                    JOptionPane.showMessageDialog(input, "Error parsing time: " + pe.getMessage(), "Error parsing time",
                            JOptionPane.ERROR_MESSAGE);
                    return false;
                }
            }
