...
export class ParameterChartTab implements OnDestroy {
  ...
  constructor(route: ActivatedRoute, private yamcs: YamcsService, private dialog: MatDialog, synchronizer: Synchronizer) {
    ...
    this.parameter$ = yamcs.yamcsClient.getParameter(yamcs.getInstance(), qualifiedName);
  ...
  compareParameter() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.yamcs.yamcsClient.getParameter(this.yamcs.getInstance(), result.qualifiedName).then(parameter => {
